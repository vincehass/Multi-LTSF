"""
Leaves one dataset out, includes all others / others specified by --train_datasets.

Trains a Lag-LLama with specific hyperparameters.
Performs early stopping with the average test error (using the Gluon prescribed test sets) of \
the respective datasets used during training.
Finally performs CRPS-Sum evaluation on the test set of the leaveoneout dataset.
"""
import warnings
...
# from data.dataset_list import OLD_DATASET_NAMES as DATASET_NAMES # use import OLD_DATASET_NAMES as DATASET_NAMES if you would like to use the datasets used previously in the workshop paper
# import sys
...
from utils.utils import plot_forecasts, print_gpu_stats, set_seed
from get_model import get_model_estimator
...
def train(args):
    ...
    if args.ckpt_path:
        ...
        checkpoint_dir_for_ckpt_path = os.path.join(fulldir_experiments_for_ckpt_path, "checkpoints")
        ckpt_path = None
        for file in os.listdir(checkpoint_dir_for_ckpt_path):
            if "best" in file:
                ckpt_path = checkpoint_dir_for_ckpt_path + "/" + file
                break
    ...
    estimator = get_model_estimator(args, train_args)
    ...
    if args.evaluate_only:
        ...
        if not args.single_dataset:
            ...
            # for name in args.test_datasets:
            #     test_data_id = name_to_data_id_map[name]
            #     test_data, prediction_length, total_points = create_test_dataset(
            #         name, dataset_path, window_size, debug=args.debug, data_id=test_data_id
            #     )
            #     test_datasets.append(test_data)
            #     test_datasets_num_points.append(total_points)
            #     test_datasets_num_series.append(len(test_data)) # If there is just 1 series this will fail
    ...
    for name in evaluation_datasets:  # [test_dataset]:
        ...
        test_data, prediction_length, total_points = create_test_dataset(
            name, dataset_path, window_size, debug=args.debug
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--dataset_path",
        type=str,
        default="/gpfs/alpine/csc499/proj-shared/lag-llama/datasets",
    )
    # parser.add_argument("-t", "--test_dataset", type=str, default="traffic", choices=DATASET_NAMES)
    ...
    parser.add_argument("--k_largest_amplitudes", type=int, default=1, help="Only for etsformer")
    parser.add_argument("--embed_kernel_size", type=int, default=1, help="Only for etsformer")
    parser.add_argument("--capacity_factor", type=int, default=1, help="Only for switch")
    parser.add_argument("--num_expert", type=int, default=1, help="Only for switch")
    parser.add_argument("--depth", type=int, default=1, help="Only for perceiver_ar")
    parser.add_argument("--hidden_size", type=int, default=1, help="Only for perceiver_ar") #cross_attn_dropout
    parser.add_argument("--cross_attn_dropout", type=int, default=1, help="Only for perceiver_ar")
    parser.add_argument("--perceive_max_heads_process", type=int, default=1, help="Only for perceiver_ar")
    parser.add_argument("--ff_mult", type=int, default=1, help="Only for perceiver_ar")
    ...
    parser.add_argument(
        "--wandb_mode", type=str, default="offline", choices=["offline", "online"]
    )
    ...
    # Model
    parser.add_argument('--model', type=str, default="lag_llama", choices=["lag_llama", "lag_transformer", "informer", "switch"])
