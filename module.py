...
class InformerModel(nn.Module):
    ...
    def get_lagged_subsequences(
        ...
        assert max(self.lags_seq) + subsequences_length <= sequence_length, (
            f"lags cannot go further than history length, found lag {max(self.lags_seq)} "
            f"while history length is only {sequence_length}"
        )

        lagged_values = []
        for lag_index in self.lags_seq:
    ...
    def forward(
        ...
        encoder_inputs, loc, scale, static_feat = self.create_network_inputs(
             past_target, past_observed_values, past_time_feat,)
        ...
        if self.time_feat:
            ...
            repeated_features = expanded_static_feat.repeat_interleave(repeats=num_parallel_samples, dim=0)
